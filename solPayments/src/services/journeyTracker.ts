...
class JourneyTracker {
   ...
   */
  async trackIncompleteUser(userData: ClientData, reason: string = 'unsupported_state') {
    if (!this.enabled) return;

    try {
      console.log('📋 [Journey Tracker] Incomplete user:', reason);
      console.log('📋 [Journey Tracker] User data:', {
      ...
      await this.sendToBackend('/track-incomplete', {
        reason,
        user_data: {
          response_id: userData.response_id,
          email: userData.email,
          preferred_name: userData.preferred_name,
          first_name: userData.first_name,
          last_name: userData.last_name,
          state: userData.state,
          selected_state_name: userData.selected_state_name || userData.state,
          payment_type: userData.payment_type,
          utm_source: userData.utm_source,
          utm_medium: userData.utm_medium,
          utm_campaign: userData.utm_campaign,
          what_brings_you: userData.what_brings_you,
      ...
      console.error('Failed to track incomplete user:', error);
    }
  }
