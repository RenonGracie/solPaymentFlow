...
export default function MatchedTherapist({
  ...
  // NEW: Get filtered available slots count for calendar coloring
  const getDayFilteredSlotCount = useCallback((date: Date): number => {
    const email = therapist?.calendar_email || therapist?.email;
    if (!email) return 0;
    ...
    if (!dayData || !dayData.available_slots) return 0;

    // Apply same filtering logic as getAvailableTimeSlots
    const now = new Date();
  ...
  }, [therapist, dailyAvailabilityCache]);
  ...
  useEffect(() => {
    ...
    // Fix: Tomorrow is the minimum booking time (not day after tomorrow)
    const minimumBookingTime = new Date(tomorrow);
    minimumBookingTime.setHours(0, 0, 0, 0);
                        ...
                        if (cell.inMonth) {
                          if (isOutsideBookingWindow) {
                            ...
                            if (isLoading) {
                              ...
                              // Use filtered slot count instead of raw API data
                              const filteredSlotCount = getDayFilteredSlotCount(cell.date);
                              if (filteredSlotCount > 0) {
                                bgClass = filteredSlotCount > 5 ? 'bg-green-100' :
                                         filteredSlotCount > 2 ? 'bg-yellow-100' : 'bg-red-100';
                              } else {
                                bgClass = 'bg-gray-200';
                                isUnavailable = true;
                              }
