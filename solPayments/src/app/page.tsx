...
// import TherapySurvey from "@/components/TherapySurvey";
...
//   // Helper function to safely obtain UTM parameters
...
//     // Prepare initial data from the insurance/cash modal
...
//       paymentType: selectedPaymentType || 'cash_pay',
//       utm: getUtmParams() // Use your existing getUtmParams function
//     };
  
//     const handleSurveySubmit = async (surveyData: any) => {
//       try {
//         // Submit to your new survey Lambda endpoint
//         const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/survey/submit`, {
//           method: 'POST',
//           headers: {
...
//         // Use the response_id just like you would from Typeform
//         if (result.response_id) {
//           await handleTypeformSubmit(result.response_id);
//         }
//       } catch (error) {
...
// solPayments/src/app/page.tsx - Updated to use CustomSurvey
"use client";
      ...
      >
        {isSearchingAnotherTherapist ? (
          ...
          <div className="min-h-screen" style={{ backgroundColor: '#FFFBF3' }}>
            ...
            {currentStep === STEPS.MATCHED_THERAPIST && (
              <div className="text-center py-20">
                <h2 className="text-2xl font-bold mb-4">We found your therapist matches!</h2>
                <p className="text-gray-600 mb-4">
                  Found {matchData?.therapists?.length} therapists for {selectedPaymentType === 'cash_pay' ? 'cash pay' : 'insurance'} clients
                </p>
                <div className="max-w-4xl mx-auto px-6">
                  {matchData?.therapists?.slice(0, 3).map((match, index) => (
                    <div key={match.therapist.id} className="bg-white p-6 rounded-lg shadow-md mb-4">
                      <h3 className="text-xl font-bold">{match.therapist.intern_name}</h3>
                      <p className="text-gray-600">{match.therapist.program} Program</p>
                      <p className="text-sm text-gray-500">Match Score: {(match.score * 100).toFixed(0)}%</p>
                      <p className="mt-2">{match.therapist.biography?.substring(0, 200)}...</p>
                      <div className="mt-2">
                        <strong>Matching Specialties:</strong> {match.matched_diagnoses_specialities.join(', ') || 'General therapy'}
                      </div>
                    </div>
                  ))}
                </div>
                {/* Here you would add the full MatchedTherapist component */}
              </div>
            )}
