// page.tsx - Updated version with new onboarding flow
"use client";
...
export default function Home() {
  ...
  const handleSelectPaymentType = (type: "insurance" | "cash_pay") => {
    ...
    setOnboardingStep(3); // Remember we're at the selection step
    
    // For insurance, show the insurance modal
    // For cash pay, go directly to the survey
    if (type === "insurance") {
      setIsInsuranceModalOpen(true);
    } else {
      // Cash pay goes directly to survey
  ...
  // Insurance Verification Modal (for insurance flow)
  return (
    <>
      <InsuranceVerificationModal
        ...
        onClose={() => {
          setIsInsuranceModalOpen(false);
          setShowOnboarding(true); // Return to onboarding at step 3 (selection screen)
        }}
        onContinueToQuestionnaire={handleModalContinue}
        paymentType="insurance"
        initialState="insurance-form"
      />
    </>
  );
} 